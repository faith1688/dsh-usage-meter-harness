...
function classifyBucket(text) {
    if (!/tokens/i.test(text))
        return null;
    if (/缓存命中/.test(text) && !/未命中/.test(text))
        return 'cacheRead';
    if (/未命中/.test(text))
        return 'input';
    if (/输出/.test(text))
...
function stripSup(text) {
    return text.replace(/\s*\(\d+\)\s*/g, '').replace(/\s+/g, ' ').trim();
}
export function parsePricingTable(tableHtml) {
    const warnings = [];
    ...
    if (named.length !== nModels)
        warnings.push(`${nModels - named.length} empty model header cell(s)`);
    ...
    for (let i = priceRowIdx; i < grid.length; i++) {
        ...
        if (priceCells.length === 0)
            break; // left the price block (or label-only row)
    ...
    for (const name of named) {
        const rec = prices[name];
        if (rec === undefined) {
            warnings.push(`${name}: no price rows at all`);
            continue;
        }
        const missing = [];
        for (const period of ['peak', 'offPeak']) {
            for (const key of ['inputPerM', 'outputPerM', 'cacheReadPerM']) {
                if (!(rec[period][key] > 0))
                    missing.push(`${period}.${key}`);
            }
        }
        if (missing.length > 0) {
...
export function extractPeakWindows(pageHtml) {
...
// ── fetch ────────────────────────────────────────────────────────────────────
export async function fetchPricingPage(url = DEEPSEEK_PRICING_PAGE_URL, timeoutMs = 25000) {
...
export function parsePricingPageHtml(html) {
    ...
    if (peak === null)
        parsed.warnings.push('footnote (3) peak-window sentence not found — keep existing local window');
    return { ...parsed, peak };
}
/** Full pipeline: fetch page → parse table + peak window. */
export async function fetchParsedPricingPage(url) {
    const html = await fetchPricingPage(url);
    return { ...parsePricingPageHtml(html), fetchedAt: Date.now() };
}
function buildLLMPricingPrompt(url, html) {
    return [
...
function llmMinutes(v) {
    if (typeof v === 'number' && Number.isFinite(v) && v >= 0 && v <= 1439)
        return Math.round(v);
    if (typeof v === 'string') {
...
export function parseLLMPricingReply(content) {
    ...
    let doc;
    try {
        doc = JSON.parse(content.slice(start, end + 1));
    }
    catch {
        return null;
    }
    const obj = doc;
    if (Array.isArray(obj.models) === false)
        return null;
    const warnings = [];
    const prices = {};
    const pageModels = [];
    for (const raw of obj.models) {
        ...
        const pk = raw.peak === null || raw.peak === undefined ? null : raw.peak;
        ...
        if (hasPeak === false)
            warnings.push(`llm: model ${name} — no parseable peak price, kept flat price`);
        prices[name] = {
            inputPerM: offIn,
            outputPerM: offOut,
            cacheReadPerM: offCache,
            peak: hasPeak
                ? { inputPerM: pkIn, outputPerM: pkOut, cacheReadPerM: llmNum(pk.cacheReadPerM) ?? 0 }
    ...
    let peak = null;
    const ph = obj.peakHours === null || obj.peakHours === undefined ? null : obj.peakHours;
    if (ph !== null) {
        const rawDays = Array.isArray(ph.days) ? ph.days : [];
        const days = rawDays.filter((d) => typeof d === 'number' && d >= 1 && d <= 7);
        const windows = [];
 ...
 */
export async function extractPricesViaLLM(apiKey, url, html, timeoutMs = 60000) {
    if (apiKey === '' || html === '')
        return null;
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), timeoutMs);
