...
function secretKey() {
    ...
    let salt;
    try {
        if (existsSync(SECRET_SALT_PATH()))
            salt = readFileSync(SECRET_SALT_PATH(), 'utf8');
        else {
            salt = randomBytes(16).toString('hex');
            writeFileSync(SECRET_SALT_PATH(), salt, 'utf8');
        }
    }
    catch {
...
function statsPath() { return join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'usage-meter', 'stats.json'); }
function saveStats() {
    if (!statsDirty)
        return;
    statsDirty = false;
...
function savePersistedConfig() {
    try {
        ...
        const payload = { providers: providerConfigs, priceOverrides, balances, thresholds, globals };
...
let activeWrapperMarkers = [' (modlens vision)', ' (vision)', '-vision', 'vision-toolkit-'];
function wrapperMarkerList(cfg) {
...
function stripWrappers(name, markers) {
    let out = name.trim();
    for (const m of markers) {
        ...
        else if (lo.endsWith(lm)) {
            out = out.slice(0, Math.max(0, out.length - m.length)).trim();
        }
    }
    return out;
}
...
export function apply(ctx, config = {}) {
    ...
    ctx.webServer.register({
        ...
        handler: async (req, res) => {
            ...
            if (req.method === 'POST') {
                ...
                if (patch.wrapperMarkers !== undefined && typeof patch.wrapperMarkers === 'string')
                    globalPatch.wrapperMarkers = patch.wrapperMarkers;
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (event.type === 'turn/start' && isDeepSeekProvider(provider))
