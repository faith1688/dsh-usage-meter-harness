...
function stampOpenTurn(turns, provider, model, at) {
    const open = turns[turns.length - 1];
    if (open === undefined || open.endedAt !== 0)
        return turns;
    if (provider === null && model === null)
        return turns;
    // 已产生过计费用量 → 本回合归属已定。
    if (open.input !== 0 || open.output !== 0 || open.cacheRead !== 0 || open.cacheWrite !== 0)
        return turns;
    if (open.provider === provider && open.model === model)
        return turns;
    ...
    apply(state, event) {
        try {
            ...
            if (event.type === 'request/header') {
                ...
                if (stamped !== next.turns)
                    next = { ...next, turns: stamped };
            }
            ...
            if (event.type === 'model/selection') {
                ...
                if (p !== null && m !== null) {
                    if (p !== state.provider || m !== state.model)
                        next = { ...next, provider: p, model: m };
...
export function apply(ctx, config = {}) {
    ...
    const readService = (name) => {
        ...
        if (typeof c.get === 'function') {
            ...
            catch { /* fall through to the property read */ }
        }
        try {
            return c[name];
        }
    ...
    const httpServer = readService('webServer');
