...
export const DEFAULT_WRAPPER_ROWS = [
...
export function wrapperRows(raw) {
    ...
    if (typeof raw === 'string') {
        ...
        for (const d of DEFAULT_WRAPPER_ROWS) {
            if (!rows.some((r) => r.pattern === d.pattern))
                rows.push({ ...d });
        }
        return rows;
    }
...
export function stripAllWrappers(name, patterns) {
    let out = name.trim();
    for (let i = 0; i < 4; i++) {
...
/** 提供商 id 归一：剥掉包装标记。 */
export function normalizeProviderId(provider, patterns) {
    if (provider === null)
        return null;
...
export function normalizeBindingKey(key, patterns) {
    if (key.startsWith('p:')) {
...
export function normalizePriceKey(key, patterns) {
