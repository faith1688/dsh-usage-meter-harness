...
// Type-only: declares ctx.webServer (dsh-host-webserver).
import type {} from '@deepseek-ai/dsh-host-webserver'
import type { Session, SessionEvent } from '@deepseek-ai/dsh-session'
import type { TokenUsage } from '@deepseek-ai/dsh-llm'
// Type-only: resolves the optional projection registry Context declaration.
import type {} from '@deepseek-ai/dsh-session-projection'
import { BUNDLED_TABLE, currentPrices, fetchRemotePrices, fetchUsdToCny, resolvePricingForTime } from './prices/index.ts'
import type { PriceKey } from './prices/index.ts'
...
// Custom log-only events (the projection re-emits immediately on them):
//  - 'usage/balance': a fresh DeepSeek API balance snapshot.
//  - 'usage/balance-ledger': a global-ledger value (non-DeepSeek) after a
//    cost deduction or a manual edit/recharge — carries the binding key so
//    only matching sessions accept it.
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
    'usage/balance': { balance: AccountBalance | null }
    'usage/balance-ledger': { key: string; balance: number; currency: string; kind: 'deduct' | 'manual' }
  }
}
...
const runtimeConfig: { currency: string; initialBalance: number | null } = {
  currency: 'CNY',
  initialBalance: null,
}
...
const balances: Record<string, LedgerEntry> = {}

/** Per-session last usage sample (turn/step) — used to compute delta deductions. */
const lastUsageBySession = new WeakMap<Session, { turn: number; step: number; input: number; output: number; cacheRead: number; cacheWrite: number; reasoning: number }>()
/** Per-session request (step) start time, mirroring the fold's `stepStart` for the live ledger path. */
const stepStartBySession = new WeakMap<Session, { turn: number; step: number; at: number }>()
...
function balanceKeyOf(provider: string | null, model: string | null): string | null {
  ...
  if (BUNDLED_TABLE[`${provider}/${model}`] !== undefined) return `p:${provider}`
  return `m:${provider}/${model}`
}

/** Read (or lazily create) the ledger entry for a key; DeepSeek returns null. */
function ledgerOf(key: string | null, defaultCurrency: string): LedgerEntry | null {
...
function broadcastBalance(key: string, entry: LedgerEntry, kind: 'deduct' | 'manual'): void {
 ...
 */
function appendSessionEvent(session: Session, type: 'usage/balance' | 'usage/balance-ledger', data: unknown, attempt = 0): void {
  try {
...
const priceOverrides: Record<string, ModelPriceOverride> = {}

interface PersistedConfig {
  providers?: Record<string, ProviderConfig>
  priceOverrides?: Record<string, ModelPriceOverride>
  /** The global balance ledger: binding key → {balance, currency}. */
  balances?: Record<string, LedgerEntry>
}
...
function applyPriceOverrides(): void {
  for (const [key, override] of Object.entries(priceOverrides)) {
    ...
    if (base === undefined) {
      ...
      currentPrices.table.merge({ [key]: row } as Record<PriceKey, ModelPricing>)
      continue
    }
    const row: ModelPricing = {
      ...base,
...
function rowsFromPricing(base: ModelPricing): PriceRow[] {
...
function defaultUnknownRows(): PriceRow[] {
  return [
...
function priceRowsOf(provider: string | null, model: string | null): PriceRow[] {
  ...
  if (base === undefined) return defaultUnknownRows()
  return rowsFromPricing(base)
}

function loadPersistedConfig(): Partial<Config> {
  try {
    const p = configPath()
    if (!existsSync(p)) return {}
    const doc = JSON.parse(readFileSync(p, 'utf8')) as PersistedConfig & Partial<Config> & { providers?: Record<string, { currency?: string; initialBalance?: number; topUps?: { turn: number; amount: number }[] }> }
    if (doc.providers) {
...
function getProviderConfig(provider: string | null): ProviderConfig {
...
function bucketsOf(usage: TokenUsage): Buckets {
  return {
...
function usageEventOf(event: SessionEvent): { turn: number; step: number; usage: TokenUsage } | null {
...
function addToLastTurn(turns: TurnBucket[], delta: Buckets, deltaCost: number, currency: string): TurnBucket[] {
  ...
  apply(state: FoldState, event: SessionEvent): FoldState {
    ...
    if (event.type === 'request/header') {
      ...
      if (provider !== state.provider || model !== state.model) {
        ...
        if (isDeepSeekProvider(provider) && state.manualAnchor !== null) next = { ...next, manualAnchor: null }
        if (!isDeepSeekProvider(provider) && state.anchor !== null) next = { ...next, anchor: null }
      }
    }
    ...
    if (event.type === 'usage/balance' && isDeepSeekProvider(state.provider)) {
      ...
      if (bal === null) {
        if (state.anchor !== null) next = { ...next, anchor: null }
  ...
  view(state: FoldState): UsageCostValue {
    ...
    if (isDeepSeekProvider(state.provider)) {
      ...
      const manual = state.manualAnchor ?? (ledger !== undefined
        ? { currency: ledger.currency, totalBalance: ledger.balance, at: 0, kind: 'manual' as const }
        : null)
      if (manual !== null) {
        accountBalance = {
...
class UsageMeterCore {
  private cfg: Config
  private priceRefreshing: Promise<void> | null = null
  private balanceRefreshing: Promise<void> | null = null
  private rateRefreshing: Promise<void> | null = null
  ...
  async refreshBalance(): Promise<void> {
    const envKey = (globalThis as unknown as { process?: { env?: Record<string, string | undefined> } }).process?.env?.DEEPSEEK_API_KEY
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  meter.applyConfig(scope.get() as Config)
  scope.watch((next) => meter.applyConfig(next as Config))
  ...
  ctx.webServer.register({
    ...
    handler: async (req: any, res: any) => {
      ...
      if (req.method === 'POST') {
        ...
        const merged: Config = { ...meter.getConfig() }
        ...
        if (patch.model !== undefined && patch.model !== null && patch.provider !== undefined && patch.provider !== null) {
          ...
          const override = patch as unknown as ModelPriceOverride & { provider: string; model: string; reset?: boolean }
          if (override.reset === true) {
            ...
            const next: ModelPriceOverride = { ...priceOverrides[key] }
            if (override.prices !== undefined) next.prices = { ...override.prices }
            if (override.rows !== undefined) next.rows = [...override.rows]
