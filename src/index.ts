...
import { BUNDLED_TABLE, DEEPSEEK_PEAK_OFF_PEAK_FROM } from './prices-providers.ts';
import { DEEPSEEK_PRICING_PAGE_URL, extractPricesViaLLM, fetchPricingPage, parsePricingPageHtml, type ParsedPricingPage } from './pricing-page.ts';
...
import { estTokens } from './globals.ts';
import {
  currentPrices,
  fetchRemotePrices,
  fetchUsdToCny,
...
function legacySaltPath(): string {
...
function migrateToWorkdir(): void {
  try {
    mkdirSync(dataRoot(), { recursive: true });
    if (existsSync(legacyStatePath()) && !existsSync(configPath())) {
      try { copyFileSync(legacyStatePath(), configPath()); } catch { /* ignore */ }
    }
    if (existsSync(legacySaltPath()) && !existsSync(saltFilePath())) {
      try { copyFileSync(legacySaltPath(), saltFilePath()); } catch { /* ignore */ }
    }
    ...
    try {
      ...
      for (const f of readdirSync(dataRoot())) {
        if (f.startsWith('dsh-billing-') && f.endsWith('.json')) {
          ...
          if (!existsSync(dst)) { try { renameSync(join(dataRoot(), f), dst); } catch { /* ignore */ } }
        }
      }
    } catch { /* ignore */ }
  } catch { /* ignore */ }
...
function secretKey(): Buffer {
  ...
  let salt: string;
  try {
    if (existsSync(saltFilePath())) salt = readFileSync(saltFilePath(), 'utf8');
    else { salt = randomBytes(16).toString('hex'); mkdirSync(dataRoot(), { recursive: true }); writeFileSync(saltFilePath(), salt, 'utf8'); }
...
const providerConfigs: Record<string, { currency?: string; sharedBalance?: boolean; sharedApiKey?: boolean }> = {};
...
const balanceSources: Record<string, 'manual' | 'deepseek'> = {};
/** 每模型独立 DeepSeek 余额快照（内存，key = 模型安全键；重启后 turn/start 重新查询）。 */
const modelBalances: Record<string, { snapshot: BalanceSnapshot; spent: number; liveAt: number }> = {};
...
const pendingModelSpent: Record<string, number> = {};
/** 独立 key 余额查询进行中的模型安全键（防并发重复查询）。 */
const modelBalanceRefreshing = new Set<string>();
/** `m:<provider>/<model>` → 是否已填入独立 DeepSeek key（布尔，非敏感，随 config.json 持久化）。 */
const modelApiKeyFlags: Record<string, boolean> = {};
...
function loadStats(): void {
  try {
    ...
    for (const [k, v] of Object.entries(doc)) if (v !== null && typeof v === 'object') {
      const s = v as StatsBucket;
      // 兼容旧格式：补齐按类别金额字段（旧 stats.json 无这些字段）。
      if (typeof s.inputCost !== 'number') s.inputCost = 0;
      if (typeof s.cacheReadCost !== 'number') s.cacheReadCost = 0;
      if (typeof s.cacheWriteCost !== 'number') s.cacheWriteCost = 0;
      if (typeof s.outputCost !== 'number') s.outputCost = 0;
      ...
      if (s.inputCost === 0 && s.cacheReadCost === 0 && s.cacheWriteCost === 0 && s.outputCost === 0 && s.cost > 0) {
        ...
        if (pricing !== null) {
          ...
          s.inputCost = bd.input; s.cacheReadCost = bd.cacheRead; s.cacheWriteCost = bd.cacheWrite; s.outputCost = bd.output;
        }
      }
      stats[k] = s;
    }
...
function rekeyWrappedPersistedState(): boolean {
  if (activeWrapperMarkers.length === 0) return false;
  let priceKeysMoved = false;
  const moved = (k: string): string | null => {
  ...
  for (const [k, v] of Object.entries(priceOverrides)) {
    const nk = moved(k);
    if (nk === null) continue;
    if (priceOverrides[nk] === undefined) priceOverrides[nk] = v;
    delete priceOverrides[k];
    priceKeysMoved = true;
  }
  // 余额来源 / 独立 Key 标记 / 阈值：底层键优先
  const moveRecord = <T>(map: Record<string, T>): void => {
    for (const [k, v] of Object.entries(map)) {
      const nk = moved(k);
      if (nk === null) continue;
      if (map[nk] === undefined) map[nk] = v;
      delete map[k];
    }
  };
  ...
  for (const [k, v] of Object.entries(balances)) {
    ...
    if (prev === undefined) balances[nk] = v;
    else if (prev.currency === v.currency) balances[nk] = { currency: prev.currency, balance: prev.balance + v.balance };
    delete balances[k];
  }
  // 看板统计：同名底层模型的历史用量求和
  ...
  for (const [k, v] of Object.entries(stats)) {
    ...
    if (nk === k) continue;
    const prev = stats[nk];
    if (prev === undefined) { stats[nk] = v; }
    else {
      prev.requestCount += v.requestCount; prev.inputTokens += v.inputTokens; prev.outputTokens += v.outputTokens;
      prev.cacheReadTokens += v.cacheReadTokens; prev.cacheWriteTokens += v.cacheWriteTokens; prev.reasoningTokens += v.reasoningTokens;
      prev.cost += v.cost; prev.inputCost += v.inputCost; prev.cacheReadCost += v.cacheReadCost; prev.cacheWriteCost += v.cacheWriteCost; prev.outputCost += v.outputCost;
      prev.updatedAt = Math.max(prev.updatedAt, v.updatedAt);
    }
    delete stats[k];
    statsMoved = true;
  }
  if (statsMoved) statsDirty = true;
 ...
 *  仅共享余额时用 `p:<provider>`。 */
function manualLedgerKeyOf(provider: string | null, model: string | null): string | null {
  if (provider === null || model === null) return null;
  const real = underlyingProvider(provider) ?? provider;
  if (providerConfigs[real]?.sharedBalance === true) return `p:${real}`;
  return `m:${real}/${model}`;
...
function ledgerOf(
  ...
  if (key === null || key === 'p:deepseek-official') return null;
...
interface ExtraStateDoc {
  providers?: Record<string, { currency?: string; sharedBalance?: boolean; sharedApiKey?: boolean; initialBalance?: number; topUps?: Array<{ amount: number }> }>;
  ...
  /** 每模型余额来源：`m:<provider>/<model>` → 'manual' | 'deepseek'。 */
  balanceSources?: Record<string, 'manual' | 'deepseek'>;
  /** 已填入独立 DeepSeek key 的模型（`m:<provider>/<model>` → true）。 */
  ...
  try {
    ...
    if (doc.providers) {
      for (const [provider, cfg] of Object.entries(doc.providers)) {
        const pc: { currency?: string; sharedBalance?: boolean; sharedApiKey?: boolean } = {};
        if (cfg.currency !== undefined) pc.currency = cfg.currency;
        if (cfg.sharedBalance !== undefined) pc.sharedBalance = cfg.sharedBalance === true;
        if (cfg.sharedApiKey !== undefined) pc.sharedApiKey = cfg.sharedApiKey === true;
    ...
    if (doc.balanceSources) Object.assign(balanceSources, doc.balanceSources);
    if (doc.modelApiKeyFlags) Object.assign(modelApiKeyFlags, doc.modelApiKeyFlags);
...
function savePersistedConfig(): void {
  try {
    ...
    const payload: ExtraStateDoc = { providers: providerConfigs, priceOverrides, balances, balanceSources, modelApiKeyFlags, thresholds, globals };
...
function getProviderConfig(provider: string | null): { currency?: string } {
  ...
  if (real !== null && providerConfigs[real] !== undefined) return providerConfigs[real];
...
let activeWrapperMarkers: string[] = activePatterns(DEFAULT_WRAPPER_ROWS);
/** 当前生效的包装标记「行表」（含未开启行），设置页据此渲染逐行开关。 */
function wrapperRowList(cfg: Record<string, unknown>): Array<{ pattern: string; enabled: boolean }> {
...
function modelBinding(provider: string | null, model: string | null): { real: string; model: string } | null {
  if (provider === null || model === null) return null;
...
function modelSourceKey(provider: string | null, model: string | null): string | null {
  const b = modelBinding(provider, model);
  return b === null ? null : `m:${b.real}/${b.model}`;
}
/** 每模型独立 API key / 余额快照的文件名安全键。 */
function modelSafeKey(provider: string | null, model: string | null): string | null {
  const b = modelBinding(provider, model);
  return b === null ? null : `${canonicalId(b.real)}__${canonicalId(b.model)}`;
}
/** 该模型的余额来源：显式设置优先；否则 DeepSeek 路由默认官方、其余默认手动（向后兼容）。 */
function balanceSourceOf(provider: string | null, model: string | null): 'manual' | 'deepseek' {
  ...
  if (key !== null && balanceSources[key] !== undefined) return balanceSources[key];
  return 'manual';
}

// ── 每模型独立 DeepSeek API key（加密存储，与全局 key 同一套 AES/salt）──
function modelApiKeyPath(safeKey: string): string {
  return join(apikeysDir(), `${safeKey}.enc`);
}
function hasModelApiKey(safeKey: string): boolean {
  try { return existsSync(modelApiKeyPath(safeKey)); } catch { return false; }
}
function saveModelApiKey(sourceKey: string, safeKey: string, plain: string): void {
  try {
  ...
  } catch (err) { console.warn('[usage-meter] save model api key failed:', err); }
}
function loadModelApiKey(safeKey: string): string | null {
  try {
    if (!existsSync(modelApiKeyPath(safeKey))) return null;
...
function hasModelApiKeyFor(provider: string | null, model: string | null): boolean {
  const safeKey = modelSafeKey(provider, model);
  if (safeKey !== null && hasModelApiKey(safeKey)) return true;
  if (provider === null || model === null) return false;
  const real = underlyingProvider(provider) ?? provider;
  if (providerConfigs[real]?.sharedApiKey !== true) return false;
  try {
...
function loadModelApiKeyFor(provider: string | null, model: string | null): string | null {
  const safeKey = modelSafeKey(provider, model);
  const own = safeKey !== null ? loadModelApiKey(safeKey) : null;
  if (own !== null) return own;
  if (provider === null || model === null) return null;
  const real = underlyingProvider(provider) ?? provider;
  if (providerConfigs[real]?.sharedApiKey !== true) return null;
  try {
    const prefix = `${canonicalId(real)}__`;
...
function deleteModelApiKey(sourceKey: string, safeKey: string): void {
  try { rmSync(modelApiKeyPath(safeKey), { force: true }); } catch { /* ignore */ }
...
function addToLastTurn(turns: FoldTurn[], delta: { input: number; output: number; cacheRead: number; cacheWrite: number; reasoning: number }, costBy: { input: number; cacheRead: number; cacheWrite: number; output: number; total: number }, currency: string, peak: boolean): FoldTurn[] {
  ...
  apply(state: FoldState, event: { type: string; data: Record<string, unknown>; time: number }): FoldState {
      ...
      if (last === undefined || last.turn !== turn) {
        next = {
          ...
          turns: [...next.turns, {
            ...
            startedAt: event.time, endedAt: 0, endReason: null, peak: false,
    ...
    if (ue !== null) {
      ...
      if (!samePrev) {
        ...
        const bd: { input: number; cacheRead: number; cacheWrite: number; output: number; total: number } = pricing === null
...
class UsageMeterCore {
  ...
  async refreshModelBalance(provider: string | null, model: string | null): Promise<void> {
    // 官方模型恒用全局 key（不读模型独立 key）。
    if (isDeepSeekProvider(provider)) { void this.refreshBalance(); return; }
    const safeKey = modelSafeKey(provider, model);
    if (safeKey === null) return;
    // 模型自有 key 优先；提供商开启「共享 API Key」时回退到共享 key。
    const apiKey = loadModelApiKeyFor(provider, model);
    if (apiKey === null) return; // 非官方无独立 key → 未配置（不走全局 key）。
    if (modelBalanceRefreshing.has(safeKey)) return;
    modelBalanceRefreshing.add(safeKey);
    try {
      const snap = toSnapshot(await fetchDeepSeekBalance(apiKey));
      if (snap === null) { delete modelBalances[safeKey]; return; }
...
export function apply(ctx: Context, config: Record<string, unknown> = {}): void {
  ...
  loadPersistedConfig();
  loadStats();
  if (rekeyWrappedPersistedState()) applyPriceOverrides();
  ...
  if (typeof (scope.get() as Record<string, unknown>).wrapperMarkers === 'string') {
    const rows = wrapperRowList(scope.get() as Record<string, unknown>);
  ...
  ctx.webServer.register({
    ...
    handler: async (_req: unknown, res: { writeHead: (s: number, h: Record<string, string>) => void; end: (s: string) => void }) => {
      ...
      try {
        const rawKey: unknown = meter.getConfig().deepseekApiKey;
        ...
        for (const p of llm.listProviders()) {
          ...
          for (const m of models) {
            ...
            if (mid === '' || local.some((x) => x.model === mid)) continue;
        ...
        const updated: Array<{ pageModel: string; model: string; input: number; output: number; cacheRead: number; peakInput: number; peakOutput: number; peakCacheRead: number }> = [];
        const missing: Array<{ pageModel: string; candidates: string[] }> = [];
        ...
        for (const [pageModel, pp] of Object.entries(page.prices)) {
          ...
          const existing: Partial<ModelPricing> | undefined = priceOverrides[key]?.prices ?? (BUNDLED_TABLE as Record<PriceKey, ModelPricing>)[key as PriceKey];
          const prices: Partial<ModelPricing> = {
  ...
  ctx.webServer.register({
    ...
    handler: async (_req: unknown, res: { writeHead: (s: number, h?: Record<string, string>) => void; end: (s?: string) => void }) => {
      ...
      try {
        ...
        const byBase = new Map<string, { provider: string; label: string; models: Array<{ model: string; label: string }> }>();
        for (const p of llm.listProviders()) {
          ...
          if (p.id === base) entry.label = p.name;
          for (const m of models) {
            const mid = stripAllWrappers(m.model, activeWrapperMarkers);
            if (entry.models.some((x) => x.model === mid)) continue;
  ...
  ctx.effect(() => {
    ...
    const ms = Math.max(1000, (meter.getConfig().refreshIntervalMs as number | undefined) ?? 4 * 60 * 60 * 1000);
  ...
  ctx.on('session/event', (session: object, event: { type: string; data: Record<string, unknown>; time: number }) => {
    ...
    if (event.type === 'turn/start' && balanceSourceOf(provider, model) === 'deepseek') void meter.refreshModelBalance(provider, model);
    if (event.type === 'step/start') {
          ...
          if (!samePrev) {
            ...
            if (pricing !== null) {
              ...
              if (mbal !== undefined) {
                ...
                if (!modelBalanceRefreshing.has(safeKey)) void meter.refreshModelBalance(provider, model);
              }
            }
          }
        }
...
export const __testInternals = {
  ...
  modelSafeKey,
  wrapperRowList,
  get statsMap(): Record<string, StatsBucket> { return stats; },
  get thresholdsMap(): Record<string, ThreshDef> { return thresholds; },
  get providerConfigsMap(): Record<string, { currency?: string; sharedBalance?: boolean; sharedApiKey?: boolean }> { return providerConfigs; },
  get balanceSourcesMap(): Record<string, 'manual' | 'deepseek'> { return balanceSources; },
  get modelApiKeyFlagsMap(): Record<string, boolean> { return modelApiKeyFlags; },
};
