...
export interface Theme {
  id: string;
  name: string;
  // popup background / border / brand gradient endpoints
  bg: string;
  bgSoft: string;
  border: string;
  brand: string;
  ...
  text2: string;
  text3: string;
  // 字体四分类（胶囊/弹窗内的文字按类型分大类，每类颜色可自定义）：
  //   textMain  = 主文字：标题、标签、正文
  //   textModel = 模型名：胶囊/弹窗里的「provider · 模型名」（渐变起点）
  //   textValue = 数值/金额：费用、tokens 数值
  //   textSub   = 次要说明：单位、提示、分隔符
  // 默认值 = 该主题对应的基础文字色，用户可在设置里逐类覆盖。
  textMain: string;
  textModel: string;
  textValue: string;
  textSub: string;
  ...
  error: string;
  ok: string;
  // capsule (pill) defaults
  pill: { flat: string; peak: string; ringFlat: string; ringPeak: string; ringOff: string };
  // stats donut palette
  palette: string[];
  // alert tier defaults
  alert: { ok: string; near: string; over: string };
 ...
 *  any color picker. */
export function resolveTheme(theme: Theme, custom: ThemeCustom): Theme {
  const fallback = theme.customDefault;
  const pick = (val: string, defaultVal: string, ultimateFallback: string): string =>
    val && val.trim() !== '' ? val : defaultVal && defaultVal.trim() !== '' ? defaultVal : ultimateFallback;
  return {
    ...theme,
    pill: {
