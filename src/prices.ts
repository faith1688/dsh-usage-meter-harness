...
export function resolvePricingForTime(pricing: ModelPricing, now: number): ModelPricing {
  ...
  const inPeak = windows.some((w) => {
    // 退化窗口 [x,x) 是空集（畸形 override），不能拼满全天 → 视作无窗口。
    if (w.start === w.end) return false;
    if (w.start < w.end) return days.includes(bjDay) && bjMin >= w.start && bjMin < w.end;
    if (bjMin >= w.start) return days.includes(bjDay);
    if (bjMin < w.end) return days.includes((bjDay + 6) % 7);
    return false;
  });
  let out = pricing;
  if (hasLegacyPeak) {
