/**
 * The `usageCost` session-projection value type and its merge into the shared
 * {@link SessionProjectionMap}. One module owns both the shape and the
 * declaration-merge so the backend fold, the wire schema, and the client
 * readout all reference the same symbols.
 *
 * @module @deepseek-ai/dsh-usage-meter/projection
 */
...
export interface UsageCostValue {
  ...
  officialPrice: { pricing: ModelPricing; rows: PriceRow[] } | null
...
export function billedInputTokens(v: Pick<UsageCostValue, 'inputTokens' | 'cacheReadTokens' | 'cacheWriteTokens'>): number {
